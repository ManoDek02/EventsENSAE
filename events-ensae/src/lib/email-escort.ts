...
// Emails romantiques pour le système cavalier/cavalière du Gala
...
const BASE_STYLES = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .profile-message { font-size: 14px; color: #6b2737; line-height: 1.6; font-style: italic; }
  .profile-message::before { content: '"'; font-size: 20px; color: #ffb3c6; vertical-align: -4px; }
  .profile-message::after { content: '"'; font-size: 20px; color: #ffb3c6; vertical-align: -4px; }
  .btn-wrap { text-align: center; margin: 28px 0; display: flex; gap: 12px; justify-content: center; flex-wrap: wrap; }
  .btn-accept { display: inline-block; padding: 12px 28px; background: linear-gradient(135deg, #c9184a, #e05c8a); color: #ffffff !important; text-decoration: none; border-radius: 50px; font-family: Arial, sans-serif; font-size: 14px; font-weight: 700; letter-spacing: 0.02em; }
  .btn-decline { display: inline-block; padding: 12px 28px; background: transparent; color: #a4133c !important; text-decoration: none; border-radius: 50px; font-family: Arial, sans-serif; font-size: 14px; font-weight: 600; border: 1.5px solid #ffb3c6; }
  ...
  .contact-label { font-family: Arial, sans-serif; font-size: 10px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: #c9184a; margin-bottom: 8px; }
  .contact-value { font-size: 15px; font-weight: 700; color: #590d22; }
...
/* ── 1. Nouvelle candidature reçue ───────────────────────────
   Envoyé à l'auteur de la demande quand quelqu'un postule
*/
export async function sendNewProposalEmail({
    ...
    <div class="profile-card">
      ...
      <div class="profile-info">
        ...
        <div class="profile-message">${proposerMessage}</div>
      </div>
    </div>
    <p style="font-family:Arial,sans-serif; font-size:13px; color:#6b2737; text-align:center; margin-bottom:20px;">
      Cette rencontre était peut-être écrite dans les étoiles…<br/>
      Vous avez 48h pour répondre.
    </p>
    <div class="btn-wrap">
      <a href="${acceptUrl}" class="btn-accept">✨ Accepter</a>
      <a href="${rejectUrl}" class="btn-decline">Décliner</a>
    </div>
    <div class="divider"></div>
    <p class="note">Si vous acceptez, vos coordonnées seront partagées mutuellement.<br/>Votre demande sera retirée de la liste publique.</p>
  `;
...
export async function sendMatchConfirmedEmail({
    ...
    partnerPromotion, partnerEmail, partnerPhoto, eventTitle,
}: {
    to: string;
    ...
    <div class="contact-box">
      <div class="contact-label">Pour le contacter / la contacter</div>
      <div class="contact-value">${partnerEmail}</div>
    </div>
    <p style="font-family:Arial,sans-serif; font-size:13px; color:#6b2737; text-align:center; margin-bottom:24px; line-height:1.7;">
      ...
      Nous vous souhaitons une nuit mémorable au Gala ENSAE.
    </p>
    <div class="btn-wrap">
