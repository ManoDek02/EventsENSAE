...
import { Heart, X, Send, Loader2, Trash2, Users, Inbox, CheckCircle2, XCircle } from "lucide-react";
...
export default function CavaliersPage() {
    ...
    /* Formulaires */
    const [form, setForm] = useState({ title: "", message: "", photoUrl: "", genderPreference: "INDIFFERENT" });
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setForm({ title: "", message: "", photoUrl: "", genderPreference: "INDIFFERENT" });
            load();
    ...
    const handlePropose = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch("/api/escort/proposals", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ requestId: proposalTarget.id, message: proposalMsg }),
            });
            const data = await res.json();
            if (!res.ok) { setError(data.error); return; }
                ...
                <div className={styles.content}>
                                    ...
                                    <>
                                        ...
                                        <div className={styles.myStatusSub}>
                                            {pendingProposals.length > 0
                                                ? `${pendingProposals.length} candidature${pendingProposals.length > 1 ? "s" : ""} en attente de votre réponse`
                    ...
                    {currentUser?.myRequest && (
                        <div style={{
                            ...
                            border: "1.5px solid #ffb3c6", borderRadius: "999px",
                            overflow: "hidden", width: "fit-content", background: "#fff0f5",
                        }}>
                            {[
                                ...
                                <button
                                    key={tab.key}
                                    onClick={() => setActiveTab(tab.key as "demandes" | "candidatures")}
                                    style={{
                                        display: "flex", alignItems: "center", gap: 7,
                                        padding: "9px 20px",
                                        background: activeTab === tab.key
                                            ? "linear-gradient(135deg, #c9184a, #e05c8a)"
                                            : "transparent",
                                        color: activeTab === tab.key ? "#fff" : "#c9184a",
                                ...
                                >
                                    ...
                                    {tab.count > 0 && (
                                        <span style={{
                                            background: activeTab === tab.key ? "rgba(255,255,255,0.25)" : "#c9184a",
                    ...
                    {loading ? (
                        ...
                        othersRequests.length === 0 ? (
                            <div className={styles.empty}>
                                ...
                                <p className={styles.emptyText}>
                                    Soyez le premier ou la première à publier votre demande pour ce Gala.
                                    Peut-être que l&apos;amour vous attend au détour d&apos;un clic…
                                </p>
                        ...
                        proposals.length === 0 ? (
                            <div className={styles.empty}>
                                ...
                                <p className={styles.emptyText}>
                                    Votre demande est publiée — les candidatures apparaîtront ici dès qu&apos;un(e) étudiant(e) s&apos;intéressera à vous.
                                </p>
                            </div>
                        ) : (
                            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                                {proposals.map((proposal) => (
                                    <div key={proposal.id} style={{
                                        background: proposal.status === "ACCEPTED" ? "#fff0f5" : proposal.status === "REJECTED" ? "#fafafa" : "#ffffff",
                                        border: `1.5px solid ${proposal.status === "ACCEPTED" ? "#c9184a" : proposal.status === "REJECTED" ? "#e5e7eb" : "#ffb3c6"}`,
                                        borderRadius: "var(--radius-xl)",
                                        overflow: "hidden",
                                        opacity: proposal.status === "REJECTED" ? 0.6 : 1,
                                        boxShadow: "0 4px 16px rgba(180,30,60,0.07)",
                                    }}>
                                        ...
                                        <div style={{ padding: "20px 22px" }}>
                                            ...
                                            <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 14 }}>
                                                {proposal.proposer.image ? (
                                                    <Image src={proposal.proposer.image} alt="" width={52} height={52}
                                                        style={{ borderRadius: "50%", objectFit: "cover", border: "2px solid #ffb3c6", flexShrink: 0 }} />
                                                ) : (
                                                    <div style={{
                                                        width: 52, height: 52, borderRadius: "50%", flexShrink: 0,
                                                        background: "linear-gradient(135deg,#ffb3c6,#ff758f)",
                                                        display: "flex", alignItems: "center", justifyContent: "center",
                                                        fontSize: 20, border: "2px solid #ffb3c6",
                                                    }}>💫</div>
                                                )}
                                                <div style={{ flex: 1 }}>
                                                    <div style={{ fontWeight: 700, color: "#590d22", fontSize: "0.95rem", marginBottom: 2 }}>
                                                ...
                                                <span style={{
                                                    ...
                                                    background: proposal.status === "ACCEPTED" ? "rgba(201,24,74,0.1)"
                                                        : proposal.status === "REJECTED" ? "rgba(107,114,128,0.1)"
                                                            : "rgba(255,179,198,0.3)",
                                                    color: proposal.status === "ACCEPTED" ? "#c9184a"
                                                        : proposal.status === "REJECTED" ? "#6b7280"
                                                            : "#800f2f",
                                                    flexShrink: 0,
                                                }}>
                                            ...
                                            {/* Message du candidat */}
                                            <div style={{
                                                background: "#fff8fb", border: "1px solid #fde8f0",
                                                borderRadius: "var(--radius-md)", padding: "14px 16px",
                                                fontSize: "0.875rem", color: "#6b2737", lineHeight: 1.7,
                                                fontStyle: "italic", marginBottom: 16,
                                            }}>
                                                <span style={{ color: "#ffb3c6", fontSize: "1.2em" }}>&ldquo;</span>
                                                {proposal.message}
                                                <span style={{ color: "#ffb3c6", fontSize: "1.2em" }}>&rdquo;</span>
                                            </div>
                                            ...
                                            {proposal.status === "PENDING" && (
                                                <div style={{ display: "flex", gap: 10 }}>
                                                    ...
                                                    >
                                                        {actionId === proposal.id ? <Loader2 size={14} /> : <CheckCircle2 size={14} />}
                                                        Accepter
                                                    </button>
                                                    <button
                                                        className={styles.btnRomanticGhost}
                                                        style={{ flex: 1, justifyContent: "center" }}
                                            ...
                                            {proposal.status === "ACCEPTED" && (
                                                <div style={{
                                                    ...
                                                    padding: "10px 14px", background: "rgba(201,24,74,0.07)",
                                                    borderRadius: "var(--radius-md)", fontSize: "0.85rem", color: "#c9184a", fontWeight: 600,
                                                }}>
                                                    <Heart size={14} fill="#c9184a" color="#c9184a" />
                                                    Match confirmé — coordonnées envoyées par email
                                                </div>
                                            )}

                                        </div>

                                        <div style={{ padding: "8px 22px 12px", fontSize: "0.75rem", color: "#d4a0b0" }}>
            ...
            {showCreateForm && (
                <div className={styles.formOverlay} onClick={() => setShowCreateForm(false)}>
                    <div className={styles.formModal} onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className={styles.formModalSub}>
                            Partagez qui vous êtes et ce que vous recherchez pour cette soirée inoubliable.
                        </div>
                        ...
                        <form onSubmit={handleCreate}>
                            <div className={styles.formGroup}>
                                ...
                                <input className={styles.formInput} required
                                    placeholder="Ex: À la recherche d'une soirée inoubliable..."
                                    value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
                            </div>
                            <div className={styles.formGroup}>
                                <label className={styles.formLabel}>Votre message *</label>
                                <textarea className={styles.formTextarea} required rows={4}
                                    placeholder="Parlez de vous, de vos attentes, de l'ambiance que vous souhaitez..."
                            ...
                            <div className={styles.formGroup}>
                                ...
                                <select className={styles.formSelect} value={form.genderPreference}
                                    ...
                                    <option value="INDIFFERENT">Peu importe — ouvert(e) à tout</option>
                            ...
                            <div className={styles.formActions}>
                                <button type="submit" className={styles.btnRomantic}
                    ...
                    <div className={styles.formModal} onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className={styles.proposalTarget}>
                            {proposalTarget.photoUrl
                                ? <Image src={proposalTarget.photoUrl} alt="" width={40} height={40}
