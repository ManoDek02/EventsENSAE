...
export default function ScannerPage() {
    ...
    /* ─── Boucle de scan avec jsQR (import npm) ──────────────── */
    useEffect(() => {
    ...
    /* ─── Recherche manuelle ─────────────────────────────────── */
    useEffect(() => {
        if (mode !== "manual" || manualQ.trim().length < 2) {
            setManualResults([]);
            return;
        }
    ...
    const manualScan = async (ticket: TicketInfo) => {
        await handleScan(ticket.qrCode);
    };
        ...
        <div className={styles.adminPage}>
            ...
            <div className="container">
                ...
                <div className={scanStyles.eventSelect}>
                    <label className={scanStyles.eventSelectLabel}>
                        <Calendar size={14} /> Filtrer par événement (optionnel)
                    </label>
                    <select
                ...
                <div className={scanStyles.modeToggle}>
                    <button
                        className={`${scanStyles.modeBtn} ${mode === "camera" ? scanStyles.modeBtnActive : ""}`}
                        onClick={() => { setMode("camera"); resetResult(); }}
                    >
                        <Camera size={15} /> Caméra
                    </button>
                    <button
                        className={`${scanStyles.modeBtn} ${mode === "manual" ? scanStyles.modeBtnActive : ""}`}
                ...
                <div className={scanStyles.layout}>
                            ...
                            <div className={scanStyles.manualSearch}>
                                ...
                                {!manualLoading && manualQ.length >= 2 && manualResults.length === 0 && (
                                    <div className={scanStyles.noResults}>
                                        <XCircle size={20} />
                                        <span>Aucun billet trouvé pour &quot;{manualQ}&quot;</span>
                                    </div>
                                )}
                                        ...
                                        {manualResults.map((t) => (
                                            <div key={t.id} className={scanStyles.manualItem}>
                                                <div className={scanStyles.manualItemInfo}>
                                                    <div className={scanStyles.manualItemName}>
                                                        <User size={13} /> {t.user.name}
                                                    </div>
                    ...
                    <div className={scanStyles.resultArea}>
                        ...
                        {scanning && (
                            <div className={scanStyles.resultIdle}>
                                <div className={scanStyles.spinner} />
                                <p>Validation…</p>
                            </div>
                        )}
...
function StatusBadge({ status }: { status: string }) {
    const cfg: Record<string, { label: string; style: React.CSSProperties }> = {
        CONFIRMED: { label: "Confirmé", style: { background: "rgba(31,107,71,0.12)", color: "#1F6B47" } },
        SCANNED: { label: "Utilisé", style: { background: "rgba(113,113,122,0.12)", color: "#52525B" } },
        PENDING: { label: "En attente", style: { background: "rgba(184,134,26,0.12)", color: "#8C6410" } },
        CANCELLED: { label: "Annulé", style: { background: "rgba(155,38,38,0.10)", color: "#9B2626" } },
    };
