...
async function handleAccept(proposalId: string, requesterId: string | null) {
    const proposal = await prisma.escortProposal.findUnique({
        ...
        include: {
            request: {
                include: {
                    requester: { select: { id: true, name: true, email: true, filiere: true, promotion: true, image: true } },
                    event: { select: { id: true, title: true } },
                },
            },
            proposer: { select: { id: true, name: true, email: true, filiere: true, promotion: true, image: true } },
        },
    });
    ...
    // Transaction : accepter cette candidature + refuser toutes les autres + fermer la demande
    ...
    // Redirection vers la page cavaliers avec message de succès
    return NextResponse.redirect(
